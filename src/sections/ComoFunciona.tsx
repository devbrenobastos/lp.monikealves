...
export const ComoFunciona: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} className="py-[clamp(56px,9vw,80px)] md:py-[clamp(80px,10vw,140px)] bg-d-bg text-paper relative overflow-hidden">
      ...
      <div 
        ref={revealRef}
        className="max-w-content mx-auto px-5 md:px-10 relative z-10"
      >
        {/* Header Block */}
        ...
        <div className="relative">
          ...
          {/* Timeline cards stack vertically on mobile (cards on right) */}
            ...
            {steps.map((step, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Main Content Card (always right of line on mobile: pl-14) */}
                  <div className={`pl-14 md:pl-0 md:w-[44%] ${isEven ? 'md:text-left' : 'md:text-right md:pr-10'}`}>
                    <div className="bg-d-surface border border-d-line p-6 md:p-8 rounded-[16px] transition-all duration-300 hover:border-olive-h/30 group">
                      ...
                      <p className="font-sans text-[16px] md:text-body-s text-cream/70 leading-relaxed max-w-[62ch] md:ml-auto">
                        <T>{step.desc}</T>
                      </p>
                    </div>
                  </div>

                  {/* Empty Spacers for alternate grid alignment on desktop */}
